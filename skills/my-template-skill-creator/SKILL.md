---
name: my-template-skill-creator
...
2. Aggregate into benchmark using: `python -m scripts.aggregate_benchmark <workspace>/iteration-N --skill-name <name>`
...
4. Tell the user to review results in the browser

## Improving the skill

After the user reviews:
1. Generalize from the feedback — avoid overfitting to specific examples
2. Keep the prompt lean — remove instructions that don't pull their weight
3. Explain the why — LLMs respond better to reasoning than rigid rules
4. Look for repeated work across test cases — bundle shared scripts into `scripts/`

Iterate until the user is happy or feedback is empty.

## Description Optimization

After the skill is done, offer to optimize the description for better triggering accuracy by running:
...
If the `present_files` tool is available, package the skill and present it:
